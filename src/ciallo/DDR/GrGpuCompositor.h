...
class GrGpuCompositor : public GrBaseCompositor
{
public:
    using VkSurfaceCreatorFunc = std::function<VkSurfaceKHR(VkInstance)>;

    GrGpuCompositor(bool debug, int32_t width, int32_t height,
                    GrColorFormat colorFormat,
                    VkSurfaceCreatorFunc surfaceMakeFunc,
    ...
    ~GrGpuCompositor() override;

    static std::shared_ptr<GrBaseCompositor> Make(const VkSurfaceCreatorFunc& surfaceMaker,
                                                  int32_t width,
                                                  int32_t height,
                                                  GrColorFormat colorFormat,
                                                  GrBasePlatform *platform,
...
private:
    ...
    static uint32_t ToVulkanColorFormat(GrColorFormat colorFormat);

private:
    VkSurfaceCreatorFunc        fSurfaceMakeFunc;
