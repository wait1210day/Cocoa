...
GrOpenCLCompositor::GrOpenCLCompositor(int32_t width,
                                       ...
                                       GrColorFormat colorFormat,
                                       GrBasePlatform *platform)
    : GrBaseCompositor("cocoa::ciallo::GrOpenCLCompositor",
                       CompositeDevice::kOpenCL,
                       width,
                       height,
                       colorFormat,
                       platform),
                                                ...
                                                void *_user_data)
{
    ...
    logOut(LOG_ERROR) << "<OpenCL.Context> [GrBaseCompositor@"
                                      ...
                                      const std::string &deviceKeyword)
{
    ...
    if (numPlatforms == 0)
    {
        artfmt(__FUNCTION__) << "No available OpenCL platforms";
    ...
    {
        artfmt(__FUNCTION__) << "No any suitable OpenCL platforms";
        arthrow<ARTException>(new ARTException);
    }
    ...
    if (numDevices == 0)
    {
        artfmt(__FUNCTION__) << "No any available OpenCL devices";
        arthrow<ARTException>(new ARTException);
    }
    ...
    {
        artfmt(__FUNCTION__) << "No any suitable OpenCL devices";
        arthrow<ARTException>(new ARTException);
    }
...
#undef PROP_STR
    ...
    logOut(LOG_DEBUG) << "New OpenCL platform: \"" << name << "\"" << logEndl;
    ...
    if (!keyword.empty() && name.find(keyword) == std::string::npos)
        return false;
    logOut(LOG_DEBUG) << "Using platform above" << logEndl;
    return true;
}
...
{
    logOut(LOG_DEBUG) << "New OpenCL device: \"" << props.fName << "\"" << logEndl;
    ...
    if (props.fMaxImage2dWidth < this->width()
        ...
        return false;
    logOut(LOG_DEBUG) << "Using OpenCL device above" << logEndl;
...
void GrOpenCLCompositor::prepareOpenCL()
{
    ...
    if (errCode != CL_SUCCESS)
    {
        ...
        artfmt(__FUNCTION__) << "Failed to build OpenCL program";
        arthrow<ARTException>(new ARTException);
    }
...
void GrOpenCLCompositor::printProgramBuildLog(const std::string& log)
{
    ...
    for (char ch : log)
    {
        if (ch == '\n')
        {
            logOut(LOG_ERROR) << "<OpenCL.Compiler>" << buffer << logEndl;
            buffer.clear();
        }
        else
            buffer += ch;
...
void GrOpenCLCompositor::toRetChecked(::cl_int ret, const char *func, const char *cl_func)
{
    if (ret == CL_SUCCESS)
        return;
    artfmt(func) << "Failed to invoke " << cl_func << ": returned " << ret;
    arthrow<ARTException>(new ARTException);
}
