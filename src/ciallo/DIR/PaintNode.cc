...
PaintNode *PaintNode::MakeFromParent(BaseNode *parent, int32_t width, int32_t height,
                                     int32_t x, int32_t y)
{
    return new PaintNode("cocoa::ciallo::PaintNode", parent, width, height, x, y);
}

PaintNode::PaintNode(const std::string& inherit,
                     BaseNode *parent, int32_t width, int32_t height,
                     int32_t x, int32_t y)
    : BaseNode(inherit, NodeKind::kPaintNode, parent),
      fWidth(width),
      fHeight(height),
      fLeft(x),
      fTop(y),
